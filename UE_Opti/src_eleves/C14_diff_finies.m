...
methode_finite_diff = 3; % 1: avant, 2: centrée, 3: complexe
...
if True_Jacobienne
	...
	switch methode_finite_diff
	  ...
	  case 3 
	     Jres = diff_finies_complexe(@(beta) res_C14_ref(beta,Ti,Ai), beta0, ndigits);
...
for i=1:10
  ...
  [beta, norm_gradf_beta, f_beta, res, norms, k, exitflag] = GN_ref(@(beta) res_C14_ref(beta,Ti,Ai), ...
		...
		case 3
  [beta, norm_gradf_beta, f_beta, res, norms, k, exitflag] = GN_ref(@(beta) res_C14_ref(beta,Ti,Ai), ...
        @(beta) diff_finies_complexe(@(beta) res_C14_ref(beta,Ti,Ai), beta, ndigits), beta0, option); 
