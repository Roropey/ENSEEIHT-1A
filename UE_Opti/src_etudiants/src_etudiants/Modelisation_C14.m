...
%
    ...
    J_res = [-exp(-beta(2)*donnees(:,1)) (donnees(:,1).*beta(1).*exp(-beta(2).*donnees(:,1)))];
...
%
    ...
    res = residu(beta);
    J_res = J_residu(beta);
    S_beta=[0 sum(res.*(donnees(:,1).*exp(-beta(2).*donnees(:,1)))); sum(res.*(donnees(:,1).*exp(-beta(2).*donnees(:,1)))) sum(res.*(-(donnees(:,1).^2).*beta(1).*exp(-beta(2).*donnees(:,1)))) ];
    H_f = J_res'*J_res + S_beta; 
