...
public class SujetCercleTest {
	...
	@Test public void ComplementsCercleTest() {
		assertEquals(2*C1.getRayon(),C1.getDiametre(),EPSILON);
		assertEquals(2*C2.getRayon(),C2.getDiametre(),EPSILON);
		assertTrue((C1.contient(A) && C1.contient(B) && (A.distance(B) <= C1.getDiametre())));
		assertTrue((C2.contient(C) && C2.contient(D) && (C.distance(D) <= C2.getDiametre())));
	}
