...
public class Individu implements Comestible {
    ...
    public Individu(Espece espece,
        ...
        this.vitesse = vitesse + vitesse * (random.nextDouble()-0.5)*2*this.espece.getEcartTypeEspece();
        this.porteeAction = porteeAction + porteeAction * (random.nextDouble()-0.5)*2*this.espece.getEcartTypeEspece();
        this.porteeVision = porteeVision + porteeVision * (random.nextDouble()-0.5)*2*this.espece.getEcartTypeEspece();
        this.vieMax = ((int) Math.floor(vieMax + vieMax * (random.nextDouble()-0.5)*2*this.espece.getEcartTypeEspece()));
        this.vie = this.vieMax;
        this.satieteMax =((int) Math.floor( satieteMax + satieteMax * (random.nextDouble()-0.5)*2*this.espece.getEcartTypeEspece()));
        this.satiete = this.satieteMax;
        this.ferveurSeuil = ((int) Math.floor(ferveurSeuil + ferveurSeuil * (random.nextDouble()-0.5)*2*this.espece.getEcartTypeEspece()));
        this.ferveur = 0;
        this.valeurNutritive = ((int) Math.floor(valeurNutritive + valeurNutritive * (random.nextDouble()-0.5)*2*this.espece.getEcartTypeEspece()));
    ...
    /**
     * Met à jour les jauges de l'individu au terme de 1 iteration
     */
    public void iterer() {
        this.ferveur++;
        ...
        if (this.satiete <= 0 || this.vie <= 0) {
            System.out.println(this.espece.getNom()+ (this.satiete <=0 ? " : Faim" : "") + (this.vie <= 0 ? " :Veilleisse" : ""));
            this.meurt();
        }
    }
    ...
    public Position getPositionPotentielle(int[] dimensions) {
        ...
        while (!positionOK) {
            ...
            if (nx >= 2 && nx <= dimensions[0]-2 && ny >= 2 && ny <= dimensions[1]-2) {
                positionOK = true;
            }
        }
        return new Position((int) (this.position.getX() + dx), (int) (this.position.getY() + dy));
    }
     ...
     */
    public Position deplacerVers (Position position, int[] dimensions) {
        double vx = position.getX() - this.position.getX();
        double vy = position.getY() - this.position.getY();
        vx = vx / ( (double) Math.sqrt(vx*vx + vy*vy));
        vy = vy / ( (double) Math.sqrt(vx*vx + vy*vy));
        int nx = (int) Math.ceil(this.position.getX() + vx * this.vitesse);
        int ny = (int) Math.ceil(this.position.getY() + vy * this.vitesse);
        nx = nx < 2 ? 4-nx : nx;
        ny = ny < 2 ? 4-ny : ny;
        nx = nx > dimensions[0] -2?  2*dimensions[0]-4-nx: nx;
        ny = ny > dimensions[1] -2 ?  2*dimensions[1]-4-ny : ny;  
     ...
     * @param ny la nouvelle position de l'individu selon l'axe des y
     */
    public void deplacerA (Position position) {
    ...
    public void mange(Comestible c) {
        ...
        if (this.satiete > this.satieteMax) {
            this.satiete = this.satieteMax;
        }
        c.meurt();
    }
    ...
    public ArrayList<Individu> seReproduit(Individu individu) {
        ...
        if (this.ferveur > this.ferveurSeuil) {
            ...
            x_enfant = this.position.getX();
            y_enfant = this.position.getY();
   ...
   public String toString(){
       return "Nouvelle Individu en " + this.position.toString() + ", Espece : " + this.espece.getNom() + ", PorteAction/Vision :" + (int) this.porteeAction + "/" + (int) this.porteeVision + ", Vie : " + this.vie + "/" + this.vieMax + ", Sasiete : " + this.satiete + "/" + this.satieteMax;
   }
}
