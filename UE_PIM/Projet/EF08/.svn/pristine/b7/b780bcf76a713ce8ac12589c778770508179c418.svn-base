...
procedure Compresser is
    ...
    procedure Encoder_Potentiel_Fichier_Vide (Nom_Fichier : in Unbounded_String;
    ...
    exception
        when Ada.IO_Exceptions.End_Error =>
            ...
            Create (Fichier_Encode, Out_File, To_String (Nom_Fichier) & ".hff");
            Close (Fichier_Encode);

        when Ada.Text_IO.Name_Error =>
            raise Fichier_Inexistant_Exception;
    end Encoder_Potentiel_Fichier_Vide;
    

    procedure Determiner_Frequence (Nom_Fichier : in Unbounded_String;
                                    Tableau_Frequences : out T_Frequences) is        
    ...
    procedure Creer_nouv_noeud (Frequence : in Integer;
    ...
    begin
        ...
        for I in Intervalle_Ascii loop
            ...
            if Tableau_Frequences (I) /= 0 then
                ...
                Creer_nouv_noeud (Tableau_Frequences (I), I, null, null,
                                  Liste_Char.Tableau (Liste_Char.Taille));
            end if;
        end loop;
        Liste_Char.Taille := Liste_Char.Taille + 1;
        -- Créer un nouveau noeud
        Creer_nouv_noeud (0, T_Octet'Last, null, null,
                          Liste_Char.Tableau (Liste_Char.Taille));
    ...
    -- Création d'un nouveau noeud de l'arbre avec les deux noeuds
    -- de clés les plus faibles
    ...
    begin
        ...
        -- Créer le nouveau noeud intermediaire contenant les 2
        -- sa cle est la somme des deux
        Creer_nouv_noeud (Noeud_plus_faible_1.Cle + Noeud_plus_faible_2.Cle,
                          0, Noeud_plus_faible_1, Noeud_plus_faible_2,
                          Nouveau_noeud);
    ...
    procedure Avoir_Code_Arbre (Arbre : in T_Arbre;
                                Code : in T_Flux_Binaire;
                                Codage : out T_Codage;
                                Code_Fin : out T_Flux_Binaire) is
    ...
    begin
        if Arbre.Gauche = null then
            if Arbre.Cle = 0 then
                ...
                Initialiser (Codage (Arbre.Valeur));
                Ajouter_Flux (Codage (Arbre.Valeur), Code);
            end if;
        else
            -- Créer le flux pour le futur code de gauche
            Initialiser (Code_Gauche);
            Ajouter_Flux (Code_Gauche, Code);
            Ajouter_Bit (Code_Gauche, 0);

            -- Créer le flux pour le futur code de droite
    ...
    procedure Encoder_Structure_Position (Arbre : in T_Arbre;
                                          Liste_Position : in out T_Liste_Chainee;
                                          Code_Structure_Arbre : in out T_Flux_Binaire) is
    begin
        if Arbre = null then
            null;
    ...
    procedure Encoder_Position (Code_Structure_Arbre : in T_Flux_Binaire;
                                Liste_Position : in out T_Liste_Chainee;
                                Fichier_Encode : in out T_Flux_Binaire) is
        procedure Ajouter_Octet_Liste (Octet : in T_Octet) is
        begin
            Ajouter_Octet (Fichier_Encode, Octet);
        end Ajouter_Octet_Liste;
        
        procedure Ajouter_Tous_Octets_Liste is
                new Liste_Chainee_Octet.Pour_Chaque (Traiter => Ajouter_Octet_Liste);
    begin
    ...
    procedure Enregistrer_Fichier is
            new Flux_Binaire.Pour_Chaque (Traiter => Enregistrer_Octet);   
    

    procedure Encoder_Arbre (Liste_Char : in T_Tableau;
                             Liste_Position : out T_Liste_Chainee;
                             Code_Structure_Arbre : out T_Flux_Binaire;
                             Fichier_Encode : out T_Flux_Binaire) is
    begin
    ...
    procedure Encoder_Fichier (Nom_Fichier : in Unbounded_String;
...
begin
    ...
    -- Encode le fichier dans le cas particulier où il est vide
    Encoder_Potentiel_Fichier_Vide (Nom_Fichier, Est_Fichier_Vide);
    if (Est_Fichier_Vide) then
        if Affichage then
            Put_Line ("Le fichier vide a bien été encodé");
        end if;

        return; -- fin de la compression
    end if;    
    ...
    -- Afficher le déroulement si besoin
    if Affichage then
        Afficher_Frequences (Tableau_Frequences);
        Afficher_Arbre (Liste_Char.Tableau(1),
                        To_Unbounded_String(""), To_Unbounded_String(""), True);
...
exception
    ...
    when Fichier_Inexistant_Exception =>
        Put_Line("Fichier à compresser inexistant.");
