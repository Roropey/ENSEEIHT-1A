...
procedure Evaluer_Alea_LCA is
	...
	is
        ...
        procedure Comparer_Liste is new Pour_Chaque (Traiter => Comparer);
        Nombre : Integer;
        Liste : T_LCA;
    begin
        Initialiser(Liste);
        for Avancement in 1..Taille loop
            ...
            if Cle_Presente(Liste,Nombre) then
                Enregistrer(Liste,Nombre,La_Donnee(Liste,Nombre)+1);
            else
                Enregistrer(Liste,Nombre,1);
            end if;
        end loop;
        Max := 0;
        Min := Taille;
        Comparer_Liste(Liste);
