...
function [ W, V, n_ev, it, itv, flag ] = subspace_iter_v2( A, m, percentage, p, eps, maxit )
    ...
    % on calcule une seule fois A^p pour faciliter les calculs
    Ap = A^p;

    % on initialise Wr car la projection de Rayleigh-Ritz ne donnera une
    % matrice de taille constante car on ne mettra pas la même matrice Vr
    % en entrée (taille changera en fonction du nombre de vecteur ayant
    % convergés)
    Wr = zeros(1,m);
