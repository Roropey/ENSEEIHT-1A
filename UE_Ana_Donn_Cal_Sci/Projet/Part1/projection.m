...
for q = 0:n-1
    if q == 0
        X_reconstruit = individu_moyen; 
    else
        C_extrait = C(:,1:q);		% q premières composantes principales
        eigenfaces_extrait = W(:,1:q);		% q premières eigenfaces
        X_reconstruit = C_extrait * eigenfaces_extrait';
    end    
    figure(1);
    ...
    for k = 1:q
     ...
     RMSE = sqrt(mean((X_reconstruit-X).^2,'all'));
